...
#import "../ShadowsocksX-NG-RX/proxy_conf_helper_version.h"
...
int main(int argc, const char * argv[])
{
    ...
    [options setBanner:@"Usage: %s [-v] [-m pac|global|off] [-u <url>] [-p <port>] [-r <port>]", argv[0]];
    
    // Version
    [options addOption:"version" flag:'v' description:@"Print the version number." block:^{
        NSLog(@"%s", [kProxyConfHelperVersion UTF8String]);
        exit(EXIT_SUCCESS);
    }];
    ...
    [options addOption:"help" flag:'h' description:@"Show this message" block:^{
        NSLog(@"%s", [[weakOptions description] UTF8String]);
        exit(EXIT_SUCCESS);
    }];
    
    // Mode
    [options addOption:"mode" flag:'m' description:@"Proxy mode, may be: pac,global,off" argument:&mode];
    
    [options addOption:"pac-url" flag:'u' description:@"PAC file url for pac mode." argument:&pacURL];
    ...
    if (![options parseArgc:argc argv:argv error:&error]) {
        ...
        NSLog(@"%@ %s: %s",stderr, argv[0], message);
        exit(EXIT_FAILURE);
    }
    ...
    NSLog(@"pac proxy set to %s", [mode UTF8String]);
